...
\begin{itemize}
	...
	\item \textbf{OLS Cross Sectional Estimate $\hat{\lambda_{OLS}}$} Using the $\beta_i$ obtained in the first stage regression, the second stage cross sectional regression model is formulated 
	
	The ols estimate is then calculated using the well-known $\hat{\lambda_{OLS}} = (\beta'\beta)^{-1}\beta'E_T(R^{e})$ with pricing error $\hat{\alpha_{OLS}}=E_T(R^e)-\hat{\lambda_{OLS}}\beta$.
