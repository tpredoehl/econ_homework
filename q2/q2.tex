...
\begin{align} \label{(X'X)^-1}
	...
	\begin{bmatrix}
		d & -b \\ -c & a
	\end{bmatrix} \notag \\
	(X'X)^{-1} &= \frac{1}{\commentTP{T}{should be taken out of the matrix}\sum_{t=1}^{T}z_{mt}^2-(\sum_{t=1}^{T}z_{mt})^2} \begin{bmatrix} \sum_{t=1}^{T}z_{mt}^2 & -\sum_{t=1}^{T}z_{mt} \\ -\sum_{t=1}^{T}z_{mt} & T \end{bmatrix} 
\end{align}
