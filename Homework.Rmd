...
\subsection*{1.a. (i) Compute also for each sample size T the distribution of the R2s of the MC simulations with either 7 separate histograms, or one unique figure where you report on the y-axis the 5%, 10% 25%, 50%, 75%, 90% and 95% quantiles of the distributions of the simulated R2, and on the x-axis you have T = 6, 12, 60, 120, 240, 360, 480}
\subsection*{(ii) Similarly (either with histograms, or with one plot of the quantiles) report the distributions of the estimates t-statistics for the test of the null H0 : β2 = 0}
\subsection*{(iii) their empirical rejection frequencies (that is the empirical size of the tests), which is exactly the figure 14.1 in Davidson MacKinnon (2005, book).}
...
graph1 <- ggplot(MC_R2, aes(x = R2, fill = T)) +
  ...
  facet_grid( T ~ . ) +
  theme_bw() +
  ggtitle(label = TeX("Distribution of $R^2$ around the 5,10,25,50,75,90 and 95% quantiles"))
...
graph2 <- ggplot(MC_beta_reject) +
  geom_col(aes(x = as.factor(T), y = pct_reject*100)) +
  ggtitle(label = TeX('% of regressions which reject $H_0: \\beta = 0$ with $t = \\frac{\\beta - 0}{\\sigma_{\\beta}}>1.96$')) +
  ylab(label = "%") +
  xlab(label = "T")
